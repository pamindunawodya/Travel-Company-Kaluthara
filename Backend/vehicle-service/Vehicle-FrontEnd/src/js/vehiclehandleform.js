        ...
        success: (resp) => {
            if (resp.code === '200') {
                resp.data.map(cus => {
                    ...
                    $("#tblVehicle tr:last-child td img:first-child").attr('src', `data:image/png;base64,${cus.vehicle_img}`);
                    $("#tblVehicle tr:last-child td img:last-child").attr('src', `data:image/png;base64,${cus.driver_license_image}`);
                });
