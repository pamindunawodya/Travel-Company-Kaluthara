...
sendAjaxVehicleRequest = (addVehicleData) => {
    ...
    $.ajax({
        ...
        success:function (resp) {
            console.log(resp);
            // alert(resp.message);
            swal("save vehicle package details successful !", "Click the ok !", "success");
...
UpdateAjaxVehicleRequest = (updateVehicleData) => {
    ...
    $.ajax({
        ...
        success:function (resp) {
            console.log(resp);
            // alert(resp.message);
            swal("update vehicle package details successful !", "Click the ok !", "success");
        },
        error: (e) => {
            console.error(e);
            alert(e);
...
$("#btnVehicleDelete").click(function () {
    ...
    $.ajax({
        url: "http://localhost:8080/Mapping/api/vehicle?id="+vehicleid,
        type:"DELETE",
        dataType: "json",
        success:function (resp){
            // alert(resp.message);
    ...
    $.ajax({
        ...
        success: (resp) => {
            if (resp.code === '200') {
                resp.data.map(cus => {
                    console.log(resp)
                    //var row = "<tr><td>adawdawdwad</td></tr>"
                    var row = '<tr><td>' + cus.id + '</td><td>' + cus.vehicle_brand + '</td><td>' + cus.vehicle_category + '</td><td>' + cus.fuel_type + '</td><td>' + cus.hybrid_OR_nonHybrid + '</td><td>' + cus.fuel_usage + '</td><td>' + cus.seatCapacity + '</td><td>' + cus.vehicle_type + '</td><td>' + cus.driver_name + '</td><td>' + cus.driver_contact + '</td><td><img width="100"></td><td><img width="100"></td></tr>';
                    $("#tblUser tbody").append(row);

                    $("#tblUser tbody tr:last-child td:nth-child(11) > img").attr('src', `data:image/png;base64,${cus.vehicle_img}`);
                    $("#tblUser tbody tr:last-child td:nth-child(12) > img").attr('src', `data:image/png;base64,${cus.driver_license_image}`);
                });
