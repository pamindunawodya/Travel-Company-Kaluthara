...
sendAjaxguideRequest = (addGuideData) => {
    ...
    $.ajax({
        url: "http://localhost:8080/Mapping/api/guide",
        type: "POST",
        data: addGuideData,
        processData: false,
...
UpdateAjaxGuideRequest = (updateVehicleData) => {
    ...
    $.ajax({
        ...
        success:function (resp) {
            console.log(resp);
            // alert(resp.message);
            swal("update guide details successful !", "Click the ok !", "success");
        },
        error: (e) => {
            console.error(e);
...
$("#btnguideDelete").click(function () {
    ...
    $.ajax({
        url: "http://localhost:8080/Mapping/api/guide?id="+id,
        type:"DELETE",
        dataType: "json",
        success:function (resp){
            // alert(resp.message);
            swal("delete guide details successful !", "Click the ok !", "success");
        },
        error:function (e) {
            alert(JSON.parse(e.responseText).message);
...
function getAllAjaxItemReq(name) {
    $.ajax({
        url: "http://localhost:8080/Mapping/api/guide/search?guidename=" + name,
        type: "GET",
        dataType: "json",
        success: function(resp) {
            console.log(resp);
...
$('#txtguidesearch').on('keyup', () => {

    let text = $('#txtguidesearch').val();

    $.ajax({
        url: "http://localhost:8080/Mapping/api/guide/search?guidename=" + text,
        type: "GET",
        dataType: "json",
        success: function(resp) {
            console.log(resp);
...
function setData(guide){

    $('#txtName0').val(guide.guidename);
}
